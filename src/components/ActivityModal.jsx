...
export default function ActivityModal({ actividad, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-[#161820] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-4">
          <div>
            ...
            <div className="text-sm text-slate-400">{form.proyecto} · {form.cliente}</div>
          </div>
            ...
            <>
              ...
              {form.descripcion && (
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-[10px] text-slate-500 mb-1">Descripción</div>
              ...
              {form.checklist?.length > 0 && (
                <div className="bg-white/5 rounded-lg p-3">
                  <div className="text-[10px] text-slate-500 mb-2">Checklist</div>
            ...
            <>
              <div className="space-y-3">
                <div>
                  ...
                  <select value={form.estado} onChange={e => set('estado', e.target.value)} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500">
                    {ESTADOS_ACT.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-slate-400 mb-1 block">Responsable</label>
                  <select value={form.responsable || ''} onChange={e => set('responsable', e.target.value)} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="date" value={form.fechaInicio} onChange={e => set('fechaInicio', e.target.value)} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Fecha límite</label>
                    <input type="date" value={form.fechaLimite} onChange={e => set('fechaLimite', e.target.value)} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-slate-400 mb-1 block">Prioridad</label>
                  <select value={form.prioridad} onChange={e => set('prioridad', e.target.value)} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500">
                    {(PRIORIDADES || []).map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-slate-400 mb-1 block">Observaciones</label>
                  <textarea value={form.observaciones || ''} onChange={e => set('observaciones', e.target.value)} rows={2} className="w-full bg-[#0F1117] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-blue-500 resize-none" />
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setEditing(false)} className="flex-1 text-slate-400 hover:text-white text-sm py-2 rounded-lg hover:bg-white/5 transition-colors">Cancelar</button>
