...
const COLUMNAS_GENERAL = [
  { id: 'arquitectura', label: 'Arquitectura', color: 'border-[#D4A017]' },
  { id: 'diseno', label: 'Diseño 3D', color: 'border-[#B5651D]' },
  { id: 'obra', label: 'Instalaciones', color: 'border-[#A67C3D]' },
  { id: 'produccion', label: 'Producción', color: 'border-[#7A4B8C]' },
  { id: 'instalacion', label: 'Instalación', color: 'border-[#2C6E9E]' },
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-steel-muted text-sm">{filtered.length} proyectos</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setFilterDept('')} className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${!filterDept ? 'bg-white/10 border-white/20 text-white' : 'border-white/10 text-steel-muted hover:text-white'}`}>Todos</button>
          {DEPTS.map(d => {
      ...
      <div className="flex gap-3 overflow-x-auto pb-4">
        {COLUMNAS_GENERAL.map(col => {
          ...
          return (
            <div key={col.id} className={`shrink-0 w-60 bg-[#1B1E23] border-t-2 ${col.color} rounded-xl flex flex-col`}>
              <div className="px-3 py-3 border-b border-steel-line flex items-center justify-between">
                ...
                {items.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => goToProject(p.id)}
                      className={`cursor-pointer bg-[#101215] border rounded-lg p-3 hover:border-white/20 transition-all ${atrasado ? 'border-red-800/70' : 'border-steel-line'}`}>
                      ...
                      <div className="text-[10px] text-steel-faint mb-2">{p.cliente}</div>
                      <div className="flex items-center gap-1 flex-wrap mb-2">
                        <DeptChip dept={getDeptActual(p)} size="xs" />
                      </div>
                      {p.proximaAccion && <div className="text-[9px] text-flame truncate">{p.proximaAccion}</div>}
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[9px] text-steel-faint">{p.responsableGeneral}</span>
                        <span className="text-[9px] text-steel-faint">{formatFecha(p.fechaEntrega)}</span>
                      </div>
                    </div>
                  );
                })}
                {items.length === 0 && <div className="text-[10px] text-steel-faint text-center py-4">Vacío</div>}
              </div>
            </div>
          );
        })}
