  ...
  return (
    <div>
      <input value={filtro} onChange={e => setFiltro(e.target.value)} placeholder="Buscar..."
        className="w-full bg-[#101215] border border-white/10 rounded-lg text-[11px] text-white px-2 py-1 mb-1.5 focus:outline-none focus:border-flame placeholder:text-slate-700" />
      <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
        {visibles.length === 0 && <span className="text-[10px] text-steel-faint">Sin resultados</span>}
        {visibles.map(item => {
          ...
          return (
            <button key={item.valor} type="button" disabled={lleno}
              ...
              className={`text-[10px] px-2 py-1 rounded-full border transition-colors shrink-0 ${activo ? 'bg-flame/20 border-flame text-flame' : 'bg-[#1B1E23] border-white/10 text-steel-muted hover:border-white/30'} ${lleno ? 'opacity-40 cursor-not-allowed' : ''}`}>
              {item.valor}
            </button>
          );
        })}
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-[#1B1E23] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-5">
          ...
          {/* ── Info básica ── */}
          <div>
            <div className="text-xs font-bold text-steel-faint uppercase tracking-wider mb-3">Información del proyecto</div>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="text-xs text-steel-muted mb-1 block">Nombre del proyecto *</label>
              ...
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Cliente *</label>
                <input value={form.cliente || ''} onChange={e => set('cliente', e.target.value)}
                  placeholder="Ej: Gran Bay Manta"
                  className="w-full bg-[#101215] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-flame placeholder:text-steel-faint" />
              </div>
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Código PEC</label>
              ...
              {esDesdeProspecto ? (
                <div>
                  <label className="text-xs text-steel-muted mb-1 block">Vendedor</label>
                  <div className="bg-[#101215] border border-steel-line rounded-lg text-sm text-steel-muted px-3 py-2">{form.responsableGeneral || '—'}</div>
                </div>
              ) : (
                <div>
                  <label className="text-xs text-steel-muted mb-1 block">Vendedor</label>
              ...
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Diseñadora (Arquitectura)</label>
              ...
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Línea del proyecto</label>
                <select value={form.lineaProyecto || ''} onChange={e => setLineaProyecto(e.target.value)}
                  className="w-full bg-[#101215] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-flame">
                  <option value="">Seleccionar línea...</option>
                  {LINEAS.map(l => <option key={l} value={l}>{l}</option>)}
                </select>
                <p className="text-[10px] text-steel-faint mt-1">Se aplica a todos los módulos del proyecto.</p>
              </div>
              ...
              {/* Fechas */}
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Fecha de ingreso</label>
                <input type="date" value={form.fechaIngreso || ''} onChange={e => set('fechaIngreso', e.target.value)}
                  className="w-full bg-[#101215] border border-white/10 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-flame" />
              </div>
              <div>
                <label className="text-xs text-steel-muted mb-1 block">Fecha estimada de instalación</label>
              ...
              {!esDesdeProspecto && (
                <div className="col-span-2">
                  <label className="text-xs text-steel-muted mb-1 block">Link plano conceptual general</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-xs font-bold text-steel-faint uppercase tracking-wider">Módulos / Muebles</div>
                <div className="text-xs text-steel-faint mt-0.5">{totalMods} módulo{totalMods !== 1 ? 's' : ''}</div>
              </div>
              <button onClick={agregarModulo}
                className="flex items-center gap-1.5 bg-flame/20 border border-flame/40 text-flame text-xs px-3 py-1.5 rounded-lg hover:bg-flame/30 transition-colors font-medium">
                <Plus size={12} /> Agregar módulo
              </button>
            </div>

            {modulos.length === 0 && (
              <div className="bg-[#101215] border border-steel-line rounded-lg px-4 py-5 text-center text-steel-faint text-xs">
            ...
            <div className="space-y-2">
              {modulos.map((mod, i) => (
                <div key={mod.id} className="bg-[#101215] border border-white/10 rounded-xl overflow-hidden">
                  {/* Header módulo */}
                  <div className="flex items-center gap-3 px-4 py-3 cursor-pointer"
                    onClick={() => setExpandedMod(expandedMod === mod.id ? null : mod.id)}>
                    <span className="text-xs font-bold text-flame font-stamp shrink-0">#{i+1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-white">{mod.nombre || <span className="text-steel-faint">Sin nombre</span>}</div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] font-mono text-steel-faint">{mod.pec}</span>
                        ...
                        {(mod.largo || mod.profundidad || mod.alto) && (
                          <span className="text-[11px] font-semibold text-steel-muted">📐 {mod.largo || '—'}×{mod.profundidad || '—'}×{mod.alto || '—'} cm</span>
                        )}
                      </div>
                      {mod.codigo && <div className="text-[10px] font-mono text-steel-faint mt-0.5 truncate">{mod.codigo}</div>}
                    </div>
                    <button onClick={e => { e.stopPropagation(); eliminarModulo(mod.id); }}
                      className="text-steel-faint hover:text-red-400 transition-colors p-1">
                      <Trash2 size={12} />
                    </button>
                    {expandedMod === mod.id ? <ChevronUp size={14} className="text-steel-faint shrink-0" /> : <ChevronDown size={14} className="text-steel-faint shrink-0" />}
                  </div>
                  ...
                  {expandedMod === mod.id && (
                    <div className="px-4 pb-4 border-t border-steel-line pt-3 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          <label className="text-[10px] text-steel-faint mb-1 block">
                            Código PEC
                          ...
                          <input value={mod.pec} onChange={e => actualizarModulo(mod.id, 'pec', e.target.value)}
                            className="w-full bg-[#1B1E23] border border-white/10 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-flame" />
                        </div>
                        <div />
                        <div>
                          <label className="text-[10px] text-steel-faint mb-1 block">Tipo de módulo</label>
                        ...
                        <div>
                          <label className="text-[10px] text-steel-faint mb-1 block">Lado</label>
                        ...
                        {(!mod.tipoModulo || !mod.lado) ? (
                          <div className="col-span-2 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2.5 text-[11px] text-steel-faint">
                        ...
                        {categorias.includes('aereo') && (
                          <div className="col-span-2">
                            <label className="text-[10px] text-steel-faint mb-1 block">Aéreo</label>
                        ...
                        {categorias.includes('superior') && (
                          <div className="col-span-2">
                            <label className="text-[10px] text-steel-faint mb-1 block">Superior (sobre mesón)</label>
                        ...
                        {categorias.includes('lcaEquipo') && (
                          <div className="col-span-2">
                            <label className="text-[10px] text-steel-faint mb-1 block">Línea caliente · Equipo</label>
                        ...
                        {categorias.includes('lcaEquipo') && (
                          <div className="col-span-2">
                            <label className="text-[10px] text-steel-faint mb-1 block">Combustible</label>
                        ...
                        {categorias.includes('inferior') && (
                          <div className="col-span-2">
                            <label className="text-[10px] text-steel-faint mb-1 block">Inferior (bajo mesón · máx. 3) {(mod.inferior||[]).length >= 3 && <span className="text-amber-400">— límite alcanzado</span>}</label>
                        ...
                        <div className="col-span-2">
                          <label className="text-[10px] text-steel-faint mb-1 block">Dimensiones (cm)</label>
                          <div className="grid grid-cols-3 gap-2">
                            <input type="number" min={0} value={mod.largo} onChange={e => actualizarModulo(mod.id, 'largo', e.target.value)}
                              placeholder="Largo"
                              className="w-full bg-[#1B1E23] border border-white/10 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-flame placeholder:text-slate-700" />
                        ...
                        <div className="col-span-2">
                          <label className="text-[10px] text-steel-faint mb-1 block">Nombre del módulo * <span className="text-steel-faint">(se arma solo; edítalo solo si el módulo no encaja en el diccionario)</span></label>
                        ...
                        <div className="col-span-2">
                          <label className="text-[10px] text-steel-faint mb-1 block">Código técnico <span className="text-steel-faint">(referencia interna — se arma solo, no editable)</span></label>
                          {(() => {
                            const codigo = generarCodigoModulo(mod);
                            if (codigo) return (
                              <div className="w-full bg-white/[0.03] border border-white/10 rounded-lg text-xs font-mono text-steel-muted px-2 py-2 min-h-[30px] break-all">
                                {codigo}
                              </div>
                            );
                            const falta = [];
                            ...
                            return (
                              <div className="w-full bg-white/[0.03] border border-white/10 rounded-lg text-[11px] text-steel-faint px-2 py-2 min-h-[30px] flex items-center">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="col-span-2">
                          <label className="text-[10px] text-steel-faint mb-1 block">Observaciones</label>
            ...
            {/* Resumen */}
            {totalMods > 0 && (
              <div className="bg-[#101215] border border-steel-line rounded-lg px-4 py-3 flex gap-6 mt-2">
                <div><div className="text-[10px] text-steel-faint">Total</div><div className="text-lg font-bold text-white">{totalMods}</div></div>
                {element > 0   && <div><div className="text-[10px] text-steel-faint">Element</div><div className="text-lg font-bold text-purple-400">{element}</div></div>}
                {santaAna > 0  && <div><div className="text-[10px] text-steel-faint">Santa Ana</div><div className="text-lg font-bold text-blue-400">{santaAna}</div></div>}
                {equifrigo > 0 && <div><div className="text-[10px] text-steel-faint">Equifrigo</div><div className="text-lg font-bold text-yellow-400">{equifrigo}</div></div>}
              </div>
            )}
          </div>
          ...
          {!isEdit && !esDesdeProspecto && (
            <div className="bg-flame/10 border border-flame/30 rounded-lg px-3 py-2">
