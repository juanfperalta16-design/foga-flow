...
export default function App() {
  ...
  const loadData = () => {
    ...
    // Recalcular estado general de cada proyecto con la nueva lógica
  ...
  const updateProyecto = (proyecto) => {
    // Siempre recalcular estado general al guardar
    const pConEstado = { ...proyecto, estadoGeneral: calcularEstadoGeneral(proyecto) };
    const updated    = proyectos.map(p => p.id === pConEstado.id ? pConEstado : p);
  ...
  const renderPage = () => {
    switch (page) {
      ...
      // Vistas de departamento — nombres actualizados
